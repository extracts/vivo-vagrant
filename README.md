...
The vivo application will be at `/home/vagrant/src/vivo`. Mac users can log into their Vagrant box securely using this command from a Terminal session.  Windows users will want to use an SSH utility, e.g. [Putty](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html):
 ...
 * A Vagrant [shared directory](https://developer.hashicorp.com/vagrant/docs/synced-folders) is available at `/work` from the box.
 * A Vagrant [shared directory](https://developer.hashicorp.com/vagrant/docs/synced-folders) is available at `/home/vagrant/src` from the box.
...
If you are interested in running older VIVO releases there are separate branches for each released version.
 ~~~
...
## Karma
[Karma](https://github.com/usc-isi-i2/Web-Karma) is a tool for mapping raw data in various formats (CSV, XML, etc) to RDF.  To assist with using Karma to model data for VIVO, a script is included to install Karma and its dependencies.  
